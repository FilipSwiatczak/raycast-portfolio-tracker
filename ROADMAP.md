...
6. Add Property handling (Including Mortgage) as an asset class. On addition ask for Total Value, Current Equity, date of valuation and post code. Create a service to fetch the price percentage change since valuation date based on the postcode (using a property price index). ✅
   ...
   - Add Shared Ownership setting (%) — splits equity by ownership percentage after deducting reserved equity ✅
   - Add Reserved Equity setting (£) — amount excluded from the shared ownership split (e.g. extra deposit contribution owned outright) ✅
   - `adjustedEquity` field on `EquityCalculation` — final user equity after ownership adjustments ✅
   - Detail panel shows Ownership Share, Reserved Equity, and Your Equity when shared ownership is configured ✅
   - 58 new unit tests for shared ownership and reserved equity scenarios (490 total) ✅
